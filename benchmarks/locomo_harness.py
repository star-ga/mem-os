#!/usr/bin/env python3
"""LoCoMo Benchmark Harness for mem-os Recall Engine.

Evaluates mem-os recall against the LoCoMo long-term conversational memory
benchmark from Snap Research. Downloads the dataset, converts conversations
to mem-os workspace format, runs QA evaluation, and reports metrics.
...
def download_dataset(force: bool = False) -> list[dict]:
    ...
    try:
        req = urllib.request.Request(
            LOCOMO_DATASET_URL,
            headers={"User-Agent": "mem-os-benchmark/1.0"},
        )
...
# ---------------------------------------------------------------------------
# Workspace builder — convert LoCoMo conversation to mem-os format
# ---------------------------------------------------------------------------
...
def build_workspace(sample: dict, base_dir: str) -> str:
    """Convert a LoCoMo conversation sample into a mem-os workspace directory.
...
def print_results_table(metrics: dict) -> None:
    ...
    print()
    print("=" * 72)
    print("LoCoMo Benchmark Results — mem-os Recall Engine")
    print("=" * 72)
...
def main():
    parser = argparse.ArgumentParser(
        description="LoCoMo Benchmark Harness for mem-os Recall Engine"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
