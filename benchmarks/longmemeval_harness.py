#!/usr/bin/env python3
"""LongMemEval Benchmark Harness for mem-os recall engine.

Evaluates mem-os BM25 recall against the LongMemEval benchmark (ICLR 2025).
Downloads the dataset from HuggingFace, converts chat sessions into mem-os
...
# Add mem-os scripts to path for recall imports
...
def download_dataset(subset: str, cache_dir: str) -> str:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "mem-os-benchmark/1.0"})
...
def session_to_block_text(session: list[dict], session_idx: int, date: str) -> str:
    """Convert a single LongMemEval session (list of turns) into a mem-os block.
...
def build_workspace(question: dict, tmpdir: str) -> str:
    """Create a temporary mem-os workspace from a LongMemEval question's haystack.
...
def print_results_table(agg: dict):
    ...
    print("\n" + "=" * 80)
    print("LongMemEval Benchmark Results (mem-os BM25 Recall)")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="LongMemEval Benchmark Harness for mem-os recall engine"
    )
    parser.add_argument(
        "--subset",
