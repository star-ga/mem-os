#!/usr/bin/env python3
"""mem-os workspace initializer. Zero external deps.

Scaffolds directory structure and copies templates for a new mem-os workspace.
...
def init(ws: str) -> tuple[list[str], list[str]]:
    ...
    # Create mem-os.json config (never overwrite)
    config_path = os.path.join(ws, "mem-os.json")
    if not os.path.exists(config_path):
        with open(config_path, "w") as f:
            json.dump(DEFAULT_CONFIG, f, indent=2)
            f.write("\n")
        created.append("file: mem-os.json")
    else:
        skipped.append("mem-os.json")
...
def main():
    ...
    print(f"mem-os init: {os.path.abspath(ws)}")
    print()
