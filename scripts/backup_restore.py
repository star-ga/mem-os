#!/usr/bin/env python3
"""mem-os Backup & Restore CLI. Zero external deps.
...
BACKUP_FILES = ["mem-os.json", "mem-os-acl.json"]
...
class WAL:
    ...
    WAL location: workspace/.mem-os-wal/
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(description="mem-os Backup & Restore")
    sub = parser.add_subparsers(dest="command")

    # Backup
    bp = sub.add_parser("backup", help="Create workspace backup")
    bp.add_argument("workspace", help="Workspace path")
    bp.add_argument("--output", "-o", help="Output file (default: mem-os-backup-<date>.tar.gz)")

    # Export
    ep = sub.add_parser("export", help="Export blocks as JSONL")
    ep.add_argument("workspace", help="Workspace path")
    ep.add_argument("--output", "-o", help="Output file (default: mem-os-export-<date>.jsonl)")
    ...
    if args.command == "backup":
        ...
        output = args.output or f"mem-os-backup-{datetime.now().strftime('%Y%m%d-%H%M%S')}.tar.gz"
        path = backup_workspace(ws, output)
        print(f"Backup created: {path} ({os.path.getsize(path)} bytes)")

    elif args.command == "export":
        ws = os.path.abspath(args.workspace)
        output = args.output or f"mem-os-export-{datetime.now().strftime('%Y%m%d-%H%M%S')}.jsonl"
