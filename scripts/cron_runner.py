#!/usr/bin/env python3
"""mem-os Cron Runner — single entry point for all periodic jobs. Zero external deps.

Reads workspace config (mem-os.json) for feature toggles, then dispatches
...
def load_config(workspace: str) -> dict:
    """Load mem-os.json from workspace. Returns defaults if missing/unreadable."""
...
def print_cron_instructions(workspace: str) -> None:
    ...
    print("# mem-os auto-ingestion (add to crontab with: crontab -e)")
    print(f"0 */6 * * * python3 {script_path} {ws} --job transcript_scan 2>&1 | logger -t mem-os")
    print(f"0 3 * * * python3 {script_path} {ws} --job entity_ingest 2>&1 | logger -t mem-os")
    print(f"0 3 * * * python3 {script_path} {ws} --job intel_scan 2>&1 | logger -t mem-os")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="mem-os periodic job runner")
    ...
    print(f"\nmem-os cron: {ok} ok, {failed} failed, {skipped} skipped (of {len(results)} jobs)")
    for r in results:
