#!/usr/bin/env python3
"""mem-os Multi-Agent Namespace & ACL Engine. Zero external deps.
...
Namespace structure:
    ...
    │   │   └── memory/
    │   └── <agent-id>/
    └── mem-os-acl.json            # Access control

ACL format (mem-os-acl.json):
    {
        "default_policy": "read",
        "agents": {
...
class NamespaceManager:
    ...
    def _load_acl(self) -> dict:
        ...
        acl_path = os.path.join(self.workspace, "mem-os-acl.json")
...
def init_multi_agent_workspace(workspace: str, agents: list[str] | None = None) -> dict:
    ...
    acl_path = os.path.join(workspace, "mem-os-acl.json")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="mem-os Namespace Manager")
